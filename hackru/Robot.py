...
class Robot(object):
    ...
    def right_smooth(self, speed, steer, seconds=None):
        """Spin to the right at the specified speed.  Will start spinning and
        return unless a seconds value is specified, in which case the robot will
        spin for that amount of time and then stop.
        """
        # Set motor speed and move both forward.
        self._left_speed(speed - steer)
        self._right_speed(speed) # Does the turning.
        self._left.run(Adafruit_MotorHAT.FORWARD)
        self._right.run(Adafruit_MotorHAT.FORWARD)
        # If an amount of time is specified, move for that time and then stop.
        if seconds is not None:
    ...
    def left_smooth(self, speed, seconds=None):
        """Spin to the left at the specified speed.  Will start spinning and
        return unless a seconds value is specified, in which case the robot will
        spin for that amount of time and then stop.
        """
    ...
    def smooth_turn(self, speed, steer, seconds=None):
        ...
        if (steer < 0): # Right turn. Right wheel moves slower.
            ...
            print("right: " + str(lesserSpeed))
            self._left_speed(speed)
            self._right_speed(lesserSpeed)
        else: # Left turn. Left wheel moves slower.
            print("LEFT TURN");
            print("left: " + str(lesserSpeed))
            print("right: " + str(speed))
            self._left_speed(lesserSpeed)
            self._right_speed(speed)
        self._left.run(Adafruit_MotorHAT.FORWARD)
        self._right.run(Adafruit_MotorHAT.FORWARD)
        # If an amount of time is specified, move for that time and then stop.
    ...
    def right_deg(self, speed, degrees):
        if (speed == 0):
            return
        seconds_to_turn = ((60.0) * (degrees / 360.0)) / speed
        #print("right_deg: " + str(seconds_to_turn))
        self.right(speed, seconds_to_turn)

    def left_deg(self, speed, degrees):
        if (speed == 0):
            return
        seconds_to_turn = ((60.0) * (degrees / 360.0)) / speed
        #print("left_deg: " + str(seconds_to_turn))
        self.left(speed, seconds_to_turn)
